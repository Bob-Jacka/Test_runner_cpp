 ...
 */
std::vector<TA::Test_case> Strategy::High_prior_strat::doAlgorithm(std::vector<TA::Test_case> &input) const {
    auto filter_func = [&](const TA::Test_case &elem) -> bool {
        const auto priority = elem.get_priority();
        const auto severity = elem.get_severity();
        return (priority == TA::Priority::High or priority == TA::Priority::Critical) and (static_cast<int>(severity) >= 1);
    };
    auto view = std::ranges::filter_view{input, filter_func};
    std::vector<TA::Test_case> to_return{};
