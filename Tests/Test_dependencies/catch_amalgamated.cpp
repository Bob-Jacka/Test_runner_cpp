...
namespace Catch {
    namespace TestCaseTracking {
        ...
        TrackerBase::TrackerBase(NameAndLocation &&nameAndLocation, TrackerContext &ctx,
                                 ITracker *parent): ITracker(CATCH_MOVE(nameAndLocation), parent),
                                                    m_ctx(ctx) {
        }
...
namespace Catch {
    namespace TextFlow {
        ...
        std::string AnsiSkippingString::substring(const_iterator begin,
            ...
            // 0xff back to 'm'
            auto str = std::string(begin == this->begin()
                                       ? m_string.begin()
                                       : begin.m_it,
                                   end.m_it);
...
namespace Catch {
    namespace Matchers {
        ...
        std::string RegexMatcher::describe() const {
            return "matches " + ::Catch::Detail::stringify(m_regex) + ((m_caseSensitivity == CaseSensitive::Yes)
                                                                           ? " case sensitively"
                                                                           : " case insensitively");
        }
...
namespace Catch {
    ...
    void defaultListTests(std::ostream &out, ColourImpl *streamColour, std::vector<TestCaseHandle> const &tests, bool isFiltered,
                          Verbosity verbosity) {
