...
namespace NJamSpell {
    ...
    /**
     * Train model with given file and alphabet
     * @param fileName txt file to train on
    ...
    bool TLangModel::Train(const std::string &fileName, const std::string &alphabetFile) {
        ...
        for (size_t i = 0; i < total; ++i) {
            ...
            for (ssize_t j = 0; j < (ssize_t) words_size - 1; ++j) {
                TGram2Key key(words[j], words[j + 1]);
                grams2[key] += 1;
            }
            for (ssize_t j = 0; j < (ssize_t) words_size - 2; ++j) {
